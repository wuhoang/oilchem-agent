# OilChem Agent 项目状态报告 (v2.1.3)

> 更新：2026-08-18
> v2.1.3 变更：DB CRUD 字段白名单（experiments 状态/审核字段禁止直接修改，防止绕过审核流程），新增 DB 白名单测试 5 个。
...
| 组件 | 文件 | 状态 | 说明 |
|---|---|---|---|
| AgentManager | `agent/manager.py` | ✅ | v0.17.0 起主链路为原生 function calling：`chat_with_tools()` + `chat_stream_with_tools()`，模型输出结构化 tool_calls，工具结果 role="tool" 回传；含重复调用检测 + 墙钟超时 120 秒 |
| MemoryManager | `agent/memory/memory.py` | 🔧 | 纯内存（重启丢失），工具往返不写入 memory（避免污染多轮对话），滑动窗口 20 条 |
| Prompts | `agent/prompts/prompts.py` | ✅ v2.1.0 | 效率优先提示词 + 动态设备表 + 按上下文裁剪领域知识 |

**function calling 主链路（v0.17.0）**：`ToolManager.list_tools_schema()` 把 25 个工具转成 OpenAI tools 协议，模型直接输出结构化 `tool_calls`（参数由协议保证合法 JSON），工具结果以 `role="tool"` 消息回传，模型自主决定继续/重试/给最终回答。含 `max_iterations=8` 防死循环 + 重复调用检测 + 墙钟超时 120 秒。旧 Planner→Executor 链路已于 v2.1.2 清理。
...
3. ~~**旧 Planner→Executor 链路未清理**~~：已于 v2.1.2 清理（~500 行死代码删除）
4. **测试覆盖不足**：28 个测试（auth 7 + bootstrap 2 + orchestrator 8 + guardrails 6 + db 5），核心聊天循环和编排器主循环仍缺单元测试
...
| 位置 | 版本号 |
|---|---|
| `backend/pyproject.toml` | 2.1.3 |
| `backend/.env` (APP_VERSION) | 2.1.3 |
| `backend/.env.example` | 2.1.3 |
| `backend/app/core/config.py` (default) | 2.1.3 |
| `backend/app/core/constants.py` | 2.1.3 |
| `frontend/package.json` | 2.1.3 |
| `frontend/package-lock.json` | 2.1.3 |
| `frontend/src/App.tsx` | v2.1.3 |
| `docs/api.md` | 2.1.3 |

## 八、v2.1.3 变更记录

1. **DB CRUD 字段白名单**：`/api/v1/db/*` 的 insert/update 端点按表过滤可写字段——experiments 仅允许 name/operator/operator_id/protocol_id/sample_code，状态与审核字段（status/reviewed_*/review_comment/result/report_path/时间戳）禁止直接修改，只能由编排器/审核流程写入，封堵「改 status 绕过审核」的业务完整性漏洞；samples/devices 无状态机，status 保留人工维护
2. **前端数据库面板联动**：experiments 表编辑态 status 改为只读徽章、新增表单不再渲染 status 输入框，避免「改了不生效」的困惑
3. **新增测试 5 个**：status 更新被忽略、审核字段更新被忽略、普通字段更新正常、insert 携带 status 落库为默认、samples.status 不受影响；总测试 23→28
4. **版本号 2.1.2 → 2.1.3**

## 九、v2.1.2 变更记录

1. **删除旧 Planner→Executor 死代码**：planner/ 目录 + executor.py + manager.py 5 个死方法 + prompts.py 死函数，合计 ~500 行
2. **修复 datetime.utcnow() 弃用**：26 处全部替换为 datetime.now(datetime.UTC).replace(tzinfo=None)
3. **清理未使用导入和过时引用**：tables.py Column/UniqueConstraint、registry.py rct-01
4. **新增测试 14 个**：编排器集成 8 个 + 输入护栏 6 个，总测试 9→23
5. **版本号 2.1.1 → 2.1.2**
